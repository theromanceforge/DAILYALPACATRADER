...
def run(sessions, flat_early=False):
    ...
    for i, s in enumerate(sessions):
        ...
        if ready:
            ...
            pos, entry, pl, gross, trades, log = 0, 0.0, 0.0, 0.0, 0, []
            cpv = cv = 0.0
            ...
            for k, b in enumerate(bars[:-1]):
                ...
                if want != pos and qty >= 1:
                    ...
                    if pos:
                        ...
                        log[-1].update(out=bars[k + 1]["dt"], exit=fill,
                                       pl=g - (entry + fill) * qty * COST_BPS / 1e4)
                    pos, entry = want, fill
                    if want:
                        trades += 1
                        log.append({"dir": want, "qty": qty, "in": bars[k + 1]["dt"], "entry": fill})
            if pos:
                fill = bars[last_i]["c"] if not flat_early else bars[last_i + 1]["o"]
                g = pos * (fill - entry) * qty
                gross += g
                pl += g - (entry + fill) * qty * COST_BPS / 1e4
                out_t = bars[last_i]["dt"] + timedelta(minutes=30) if not flat_early else bars[last_i + 1]["dt"]
                log[-1].update(out=out_t, exit=fill, pl=g - (entry + fill) * qty * COST_BPS / 1e4)
            days.append({"date": s["date"], "pl": pl, "gross": gross, "trades": trades, "log": log})
        moves.append(slots)
